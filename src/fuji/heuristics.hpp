...
namespace UECda{
	namespace Fuji{
        class HeuristicChanger{
            // ヒューリスティック交換
        private:
            Cards pruneCards_f(const Cards, const int)const;
            Cards prune_f(Cards *const, const int, const Cards, const int)const;
        public:
            static AtomicAnalyzer<2> ana; // アナライザー
            Cards pruneCards(Cards myCards, int changeQty)const{
                Cards c;
                ana.start();
                c = pruneCards_f(myCards, changeQty);
                ana.end(0);
                ASSERT(holdsCards(myCards, c),);
                ASSERT((int)countCards(c) >= changeQty,);
                return c;
            }
            int prune(Cards *const pc, int NCards, Cards myCards, int changeQty)const{
                int n;
                ana.start();
                n = prune_f(pc, NCards, myCards, changeQty);
                ana.end(1);
                return n;
            }
        };
        
        AtomicAnalyzer<2> HeuristicChanger::ana("HeuristicChanger");
        
        Cards HeuristicChanger::pruneCards_f(const Cards argCards, int changeQty)const{
            Cards tmp = argCards;
            // 絶対に渡さないカードを抜く
            // 8, 2, Joker, D3
		...
		class HeuristicPlayer{
			// 条件別辞書式順序によるヒューリスティックプレイ
			// ...だけのはずだったのだが、着手の扱い関連で根拠不明瞭な部分のある操作は大体ここが担っている
			
			// 着手基本追加情報は既に計算されていると考えてよい
			// 返り値はインデックスの方が良いのだろうが、パスのインデックスが何か等を考えるのが面倒なため、着手そのまま
		private:
			static AtomicAnalyzer<2> ana;
			
		public:
			template<class playSpace_t, class sbjField_t>
			int pruneRootSub(playSpace_t *const ps, const int NCands, const sbjField_t& field)const;
			
			template<class playSpace_t, class sbjField_t, class dice_t>
			Move chooseMateMoveSub(playSpace_t *const ps, int NCands, const sbjField_t& field, dice_t *const pdice)const;
			...
			int pruneRoot(playSpace_t *const ps, const int NCands, const sbjField_t& field){
				...
				int res;
				ana.start();
				res = pruneRootSub(ps, NCands, field);
				ana.end(1, 0);
				return res;
			}
			
			template<class playSpace_t, class sbjField_t, class dice_t>
			Move chooseMateMove(playSpace_t *const ps, const int NCands, const sbjField_t& field, dice_t *const pdice){
				// 必勝着手の中から、どれを選ぶか決める
				Move m;
				ana.start();
				m = chooseMateMoveSub(ps, NCands, field, pdice);
				ana.end(0, 0);
				return m;
			}
		...
		AtomicAnalyzer<2> HeuristicPlayer::ana("HeuristicPlayer");
		
		template<class playSpace_t, class sbjField_t, class dice_t>
		Move HeuristicPlayer::chooseMateMoveSub(playSpace_t *const ps, int NCands, const sbjField_t& field, dice_t *const pdice)const{
			...
			//const FieldAddInfo fInfo = ps->getFInfo();
		...
		template<class playSpace_t, class sbjField_t>
		int HeuristicPlayer::pruneRootSub(playSpace_t *const ps, const int NCands, const sbjField_t& field)const{
			// 枝刈り(ルートノード)
			...
			const FieldAddInfo& fInfo = ps->fInfo;
			const Board& bd = field.getBoard();
			//int ret = NCands;
			const Cards remCards = field.getRemCards();
			const Cards myCards = field.getMyCards();
			const Cards opsCards = field.getOpsCards();
			const Cards DWCards = getAllDWCards(myCards, opsCards, bd.tmpOrder(), fInfo.isTmpOrderSettled());
			...
			for(int m = NCands - 1; m >= 0; --m){ // 逆向きループ
				...
				if(bd.isNF() && (!mv->changesPrmState())){
					// 空場、、永続的パラメータ変更無し
					if(hasDWorNFH(nextCards, opsCards, bd.tmpOrder(), fInfo.isTmpOrderSettled())){
				...
				if(fInfo.isTmpOrderSettled()){ // オーダー固定が必須
					if(mv->isDO()
					   && !containsJOKER(mvCards)
					   && !(containsJOKER(remCards) && containsS3(mvCards))
					   ){// 他支配あり、他諸々
