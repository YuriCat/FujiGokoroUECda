...
#ifndef UECDA_FUJI_TIME_ANALYSIS_HPP_
#define UECDA_FUJI_TIME_ANALYSIS_HPP_
...
# ifdef MODELING_TIME
        ...
        bool judgeWatchingMode(const clientGameLog_t& gLog){
            ...
            for(int t = 1; t < gLog.plays(); ++t){ // turn1から
                ...
                if(pLog.sbjTime() != 0){ // 主観的計算時間は自分以外は0のはず
                    if(pLog.time() - pLog.sbjTime() >= 1000 * 1000 / 10){ // 0.1秒以上のディレイがあった
...
#endif // UECDA_FUJI_TIME_ANALYSIS_HPP_
