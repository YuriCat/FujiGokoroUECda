...
#ifndef UECDA_FUJI_DEALER_HPP_
#define UECDA_FUJI_DEALER_HPP_
...
// TODO: WAまわりがjava版のルールの違いに対応していない
...
namespace UECda{
	namespace Fuji{
        ...
        int dist2Rest_64(uint64_t *const rest, uint64_t *const goal0, uint64_t *const goal1,
            ...
            if((!holdsBits(tmp0, rest0)) || (!holdsBits(tmp1, rest1 & (~highNRest)))){
                return 0;
            }
        ...
        public:
            ...
            void dealAllRand(Cards *const dst, dice64_t *const pdice)const{
                ...
                for(int r = 0; r < N; ++r){
                    int p = infoClassPlayer[r];
                    dst[p] = andCards(remCards, tmp[r]);
                }
                for(int p = 0; p < N; ++p){
            ...
            void dealWithBias(Cards *const dst, dice64_t *const pdice)const{
                //　逆関数法でバイアスを掛けて分配
                if(flag.test(0)){
                    return dealWithAbsSbjInfo(dst, pdice);
                }
                ana.start();
                ...
                uint64_t sumOfWeight[N];
                
                for(fromCards = distCards; anyCards(fromCards);){
                    IntCard ic = popIntCardHigh(&fromCards);
                    
                    if(NTmpDeal[0]){
                        sumOfWeight[0] = NTmpDeal[0] * Deal::CPTable_afterChange[ic][0];
                    }else{
                        sumOfWeight[0] = 0;
                    }
                    ...
                    for(int r = 1; r < N; ++r){
                        if(NTmpDeal[r]){
                            sumOfWeight[r] = sumOfWeight[r - 1] + NTmpDeal[r] * Deal::CPTable_afterChange[ic][r];
                        }else{
                            sumOfWeight[r] = sumOfWeight[r - 1];
                        }
                    }
                    assert(sumOfWeight[N - 1] > 0);
                    uint32_t ran = pdice->rand() % sumOfWeight[N - 1];
                    int r;
                    for(r = 0; r < N; ++r){
                        if(sumOfWeight[r] > ran){
                            break;
                        }
                    }
                    addCards(&tmp[r], convCard(ic));
                    --NTmpDeal[r];
                }
            ...
            template<class sharedData_t, class threadTools_t, class bucket_t>
            void dealBucketWithRejection(bucket_t *const dst, const sharedData_t& shared,
                                         threadTools_t *const ptools){
                // 採択棄却法メイン
                // 設定されたレートの分、カード配置を作成し、手札親和度最大のものを選ぶ
                assert(HARate >= 1 && HARate < 64);
                ...
                if(field.isInitGame()){
                    ...
                    if(!phase.isInChange() // 交換中でない
                       && field.isMyImpChange() // カード交換に関与した
                        ...
                        if(myClass > MIDDLE){ // 貧民以下
                            assert((int)countCards(recvCards) == N_CHANGE_CARDS(myClass));
                        }
                    }
        ...
        private:
            ...
            // 進行得点関連
            int turnNum;
            static constexpr uint32_t HARATE_MAX = 32;//27;//20;
            uint32_t HARate;
            ...
            // inner function
            static void clearDst(Cards *const dst){
                for(int p = 0; p < N; ++p){ dst[p] = CARDS_NULL; }
            }
            ...
            int checkCompWithRejection()const{
                ...
                if(!flag.test(0)){
                    switch(myClass){
                        case DAIFUGO:
                            if(NDet[FUGO] >= 9 || NDet[HINMIN] >= 9){ comp = 0; }break;
                        case FUGO:
                            if(NDet[DAIFUGO] >= 9 || NDet[DAIHINMIN] >= 9){ comp = 0; }break;
                        case HEIMIN:
                            if(NDet[DAIFUGO] >= 7 || NDet[FUGO] >= 7 || NDet[HINMIN] >= 8 || NDet[DAIHINMIN] >= 8){ comp = 0; }break;
                        case HINMIN:
                                ...
                                while(1){
                                    ...
                                    if(NDeal[DAIHINMIN]){
                                        ...
                                        while(1){
                                            if(tt > 10000){
                                                // 無限ループと思われる
                                                for(IntCard icc = INTCARD_MIN; icc <= INTCARD_MAX; icc){
                                                    CERR << weight[icc] << ",";
                                                }
                                                CERR << "sum:" << weightSum << endl;
                                                UNREACHABLE;
                                            }
                                            tmpR4 = CARDS_NULL;
                                            int NDeal_tmp = NDeal[DAIHINMIN];
                                            for(IntCard ic = INTCARD_MIN; ic <= INTCARD_MAX; ++ic){
                                                if(weight[ic] > 0){
                                                    int r = pdice->rand() % ((weightSum / NDeal[DAIHINMIN]) + 1);
                                                    if(r < weight[ic]){
                                                        tmpR4 |= convCard(ic);
                                                        if(NDeal_tmp == 0){ // 配り過ぎ
                                                            ++tt;
                                ...
                                while(1){
                                    ...
                                    if(NDeal[HINMIN]){
                                        ...
                                        while(1){
                                            ...
                                            tmpR3 = CARDS_NULL;
                                            int NR3Deal_tmp = NDeal[HINMIN];
                                            for(IntCard ic = INTCARD_MIN; ic <= INTCARD_MAX; ic++){
                                                if(weight[ic] > 0){
                                                    int r = pdice->rand() % ((weightSum / NDeal[HINMIN]) + 1);
                                                    if(r < weight[ic]){
                                                        tmpR3 |= convCard(ic);
                                                        if(NR3Deal_tmp == 0){ // 配り過ぎ
                                                            ++tt;
                                                            goto TO_NEXT_F;
                                                        }
                                                        NR3Deal_tmp--;
            ...
            void setWeightInWA(){
                ...
                if(T > 0){
                    ...
                    while(tmp){
                        ...
                        distCardsUnderInWA[index] = distCards & pickLower(convCard(ic));
                        index += 1;
                    }
                    ASSERT(index > 0,);
                    // WA mathod 用の配列に入れる
                        ...
                        if(myDealtCards & c){ // 自分の初期カードと一致
                            ...
                            if(underJ >= T){ // ここまでにT以上のカードがあった
                                ...
                                for(IntCard ic2 = INTCARD_MIN; ic2 < ic; ++ic2){
                                    if(distCards & convCard(ic2)){ // それ以下のカードを持つ重みを加算
            ...
            void dealPart_AbsSbjInfo(){
                ...
                if(!flag.test(0)
                   && !phase.isInChange()
                    ...
                    if(myClass < MIDDLE){ // 自分が上位のとき
                        ...
                        //Cards vZone=pickNthHigh(myDealtCards,2-myClass)-1ULL;
                ...
                if(!phase.isInChange()
                   && !phase.isFirstTurn()
                    ...
                    playFlag.reset();
                    for(int p = 0; p < N; ++p){
                        if(p != myNum && NDeal[infoClass[p]] > 0){
                            playFlag.set(p);
                        }
                    }
                }else{
            ...
            double calcPlayLikelihood(Cards *const c, const sharedData_t& shared, threadTools_t *const ptools)const{
                ...
                (shared.gameLog, orgCards,
                 ...
                 (const auto& field, const auto& chosenMove, uint32_t usedTime)->int{
                     ...
                     //cerr<<field.getTurnNum()<<" "<<chosenMove<<" "<<field.getBoard()<<endl;
                     
                     const Cards usedCards = chosenMove.c();
                     ...
                     if(!holdsCards(myCards, usedCards)){
                         return -1; // 終了(エラー?)
                     }
                     ...
                     if(tmpPlayFlag.test(tp)){
                         ...
                         if(NMoves > 1){
                             for(int m = 0; m < NMoves; ++m){
                                 bool mate = checkHandMate(0, mv + NMoves, mv[m], myHand, opsHand, bd, field.fInfo, 1, 1);
