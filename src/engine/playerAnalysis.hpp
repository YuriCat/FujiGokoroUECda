...
namespace UECda{
	namespace Fuji{
        ...
        void analyzePlayerModel(sharedData_t *const pshared,
            ...
            const auto& matchLog = pshared->matchLog;
            const auto& gameLog = matchLog.latestGame();
            
            // まず、観戦モードで動いていたかを推定
            if (judgeWatchingMode(gameLog)) {
            ...
            if (!gameLog.isTerminated()) { return; } // 試合が普通に終了した場合のみ解析
            ...
            for (int itr = 0; itr < 8; ++itr) {
                PolicyGradient::learnPlayParamsGame<1>(gameLog, flag, pmodelSpace, ptools);
            }
            ...
            if (Settings::simulationPlayModel) {
                // 確率のずれによって方策パラメータ調整
                learnPlayBiasGame(matchLog.getMyPlayerNum(),
                                  gameLog, pshared->basePlayPolicy, pmodelSpace, ptools);
            }
