...
namespace UECda {
    ...
    // L2局面表現へのチェンジ
    void convL2Field(const Board& b, const FieldAddInfo& info, L2Field *const dst) {
        dst->info = info;
        dst->b = b;
    }
    ...
    int procL2Field(const L2Field& cur, L2Field *const pnext, const MoveInfo mi) {
        ...
        if ((IS_SFOL == _YES) || cur.isUnrivaled()) { // 独壇場
            ...
            if ((IS_NF == _YES) || ((IS_NF != _NO) && cur.isNull())) {
                if (mi.dominatesAll()) {
                    pnext->b.procAndFlush(mv);
    ...
    public:
        ...
        //int start_check(const Move move, MoveAddInfo *const mvInfo, const Hand& myHand, const Hand& opsHand, const Board b, const FieldAddInfo fInfo);
    ...
    int L2Judge::judge(const int depth, MoveInfo *const mv_buf, const Hand& myHand, const Hand& opsHand, const L2Field& field) {
        ...
        switch(S_LEVEL) {
            ...
            case 3:
                ...
                ana.restart(mode, 1);
                if (IS_NF == _YES) {
                    if (judgeHandPW_NF(myHand, opsHand, field.b)) {
                        ana.restart(mode, 5);
                        return L2_WIN;
                    }
                }
            ...
            case 4:
                ...
                if ((IS_NF == _YES) || (IS_NF != _NO && field.isNull())) {
            ...
            default: // 完全探索
                ...
                const int NMoves = genMove(mv_buf, myHand, field.b);
                ...
                if (IS_NF == _NO && field.b.qty() >= myHand.qty && NMoves >= 2) { return L2_WIN; }
        ...
        switch(S_LEVEL) {
            ...
            case 2:{
                ...
                if (((IS_PD == _YES && (IS_NPDO == _YES)) || ((IS_NPDO == _YES) && (IS_PASS == _NO)) || ((IS_PD == _YES) && (IS_PASS == _YES))) // 無条件に支配
                    ||((IS_PASS == _NO) && dominatesCards(tmp.mv(), opsHand.cards, field.b))) { // 他支配チェック
                    tmp.setDomOthers();
            ...
            default:{
                ...
                if (IS_PASS == _NO) {
                    if (field.isDConst()) { // 支配拘束中
                        ...
                        // 支配保証拘束を掛ける
                        if ((IS_NF == _YES) && field.isOrderSettled() && tmp.dominatesOthers()) {
                            if (hasDWorNFH(myHand.cards - tmp.cards(), opsHand.cards, field.order(), field.info.isOrderSettled())) { // 残り札に支配保証or空場期待がある
                ...
                if (IS_PASS == _NO) {
                    ...
                    int nextL2FINFO = genTA_L2FINFO(nextField.isNull() ? _YES : _NO, nextField.isUnrivaled() ? _YES : _NO);
                    ...
                    if (!tmp.isL2GiveUp()) {
                        ...
                        if (res == L2_DRAW) {
                            ...
                            if (tmp.dominatesOthers()) {
                                ...
                                if ((IS_NF == _YES) && field.isOrderSettled()) {
