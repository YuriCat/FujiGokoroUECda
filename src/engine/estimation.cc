...
union ChangeEntry {
    ...
    static constexpr size_t vsize() { return 32; }
};
union PlayEntry {
    uint64_t data;
    struct {
...
template <size_t N>
class ChangeTable : public PolicyTable <ChangeEntry, N> {
public:
    void regist(uint64_t key, float prob) {
        ChangeEntry e;
        e.prob = prob;
        PolicyTable<ChangeEntry, N>::set(key, e);
    }
};
template <size_t N>
...
ChangeTable<(1 << 16) + 3> changeTT;
PlayTable<(1 << 18) + 3> playTT;
...
double changeProb(const int p, const Cards cards, const int qty,
    ...
    prob = selector.prob(idx);

    changeTT.regist(key, prob);
    return prob;
}
...
double RandomDealer::playLikelihood(Cards *const c, const GameRecord& game,
    ...
    tuple<double, int, uint64_t> playerPlayLikelihood[N];
    // テーブルに記録されているプレーヤーごとの尤度を調べる
    for (int p = 0; p < N; p++) {
        playerPlayLikelihood[p] = make_tuple(1.0, -1, 0);
        if (!playFlag.test(p)) continue;
        uint64_t key = CardsToHashKey(orgCards[p]) ^ p;
        const auto entry = playTT.read(key);
        if (entry.prob <= 0 || entry.player != p) continue;
        playerPlayLikelihood[p] = make_tuple(entry.prob, entry.tcnt, key);
    }
    Field field;
    ...
    // play callback
    [this, &shared, ptools, &playerPlayLikelihood]
    (const Field& field, Move move, unsigned time)->int{
        ...
        // テーブルに記録されている場合は飛ばす
        if (get<1>(playerPlayLikelihood[turn]) < tcnt) {
            double lh = onePlayLikelihood(field, move, shared, ptools);
            get<0>(playerPlayLikelihood[turn]) *= lh;
        }
        return 0;
    });
    double prod = 1;
    for (int p = 0; p < N; p++) {
        if (!playFlag.test(p)) continue;
        auto lh = playerPlayLikelihood[p];
        prod *= get<0>(lh);
        // 更新できた分の尤度をテーブルに保存
        if (turnCount > get<1>(lh)) playTT.regist(get<2>(lh), turnCount, p, get<0>(lh));
    }
    return log(prod); // 尤度を返す
}